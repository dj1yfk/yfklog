...
my $dbname = "";								# DB name
my $onlinedata = "`CALL`, `DATE`, round(`BAND`,2), `MODE`";
...
sub readw {
	...
	while (1) {										# loop infinitely
		...
		# F5 -> get frequency and mode from the transceiver
			...
			my $freq = `rigctl -m $rigmodel f 2> /dev/null`;
			my $mode = `rigctl -m $rigmodel m 2> /dev/null`;

			# chech if anything was returned. if not, rigctl cannot be found,
			# then exit.
			...
			# in some cases, an error (get_freq: error..) occurs. if so, stop
			# and exit
...
sub lastqsos {
	...
	my $l = $dbh->prepare("SELECT `CALL`, `BAND`, `MODE`, `DATE`, `T_ON`,
			`NAME`, `QTH`, `RSTS`, `RSTR`, `QSLS`, `QSLR` FROM log_$mycall
			ORDER BY NR DESC LIMIT $nr");	
	$l->execute();
	# temporary vars
	...
	while ($l->fetch()) {						# while row available
		...
		if ($screenlayout == 0) {
			...
			addstr($wlog,$y,0, 
			sprintf("%-12s%-4s %-5s%-4s  %-6s  %-8s  %-13s  %-3s %-3s  %s %s    ",
	...
	if  (defined $prefix) {	# &wpx returns undef when callsign is invalid
		...
		unless ($newdxcc eq '0E0') {			# DXCC already worked
			...
			$dx = $dbh->prepare("SELECT NR from log_$mycall WHERE 
						DXCC='$dxcc[7]' AND round(BAND,2)='$band';");
		...
		my $lq = $dbh->prepare("SELECT `CALL`, `DATE`, `T_ON`, `BAND`, `MODE`,
				`QSLS`, `QSLR`, `NAME`, `QTH`, `RSTS`, `RSTR` from log_$mycall
				WHERE `CALL` 
				REGEXP '^(.*/)?$call(/.*)?\$' ORDER BY `DATE`, `T_ON`;");
		...
		while ($lq->fetch()) {				# more QSOs available
			...
			if ($screenlayout == 0) {		
				$line = sprintf("%-14s %-8s %-5s %4s %-4s %1s %1s     ", 
					$lcall, $ldate, $ltime, $lband, $lmode, $lqsls, $lqslr);
			}
			elsif ($screenlayout ==1) {
				$line =	sprintf("%-12s%-4s %-5s%-4s  %-6s  %-8s  %-13s  %-3s %-3s  %s %s    ",
...
do  {			# loop and get keyboard input
	...
	my $cq = $dbh->prepare("SELECT `NR`, `CALL`, `BAND`, `MODE`, `DATE`,
			`T_ON`, `NAME`, `QTH`, `RSTS`, `RSTR`, `QSLS`, `QSLR` FROM
			log_$mycall ORDER BY `NR` DESC LIMIT $offset, $nbr");	
...
do {									# we start looping here
	my $lq = $dbh->prepare("SELECT `NR`, `CALL`, `DATE`, `T_ON`, `BAND`, `MODE`,
		   	`QSLS`, `QSLR`, `NAME`, `QTH`, `RSTS`, `RSTR` FROM log_$mycall
			WHERE `CALL` REGEXP '^(.*/)?$call(/.*)?\$' ORDER BY `DATE`, `T_ON`
			LIMIT $offset, $nbr");	
...
# Now the log is queried for queued QSLs.. 
	...
	my $queue = $dbh->prepare("SELECT `CALL`, `NAME`, `DATE`, `T_ON`, `BAND`, 
				`MODE`, `RSTS`, `PWR`, `QSLINFO` FROM log_$mycall WHERE
...
sub adifexport {
	...
	my $adif = $dbh->prepare("SELECT `CALL`, `DATE`, `T_ON`, `T_OFF`, `BAND`,
			...
			`PWR`, `PFX`, `CONT`, `QSLINFO`, `CQZ`, `ITUZ`, `IOTA`, `STATE` FROM
			log_$mycall"); 
...
for my $i ( 0 .. $#qso ) {					# iterate through Array of Hashes
	...
	# FIXME use <dxcc:> part of ADIF if available!
	# since lookup with the dxcc-sub is really slow, the top 10 DXCC as of my
	# log (40k QSOs) are hardcoded: DL, K, UA, G, OK, UR, UA9, SP, SM, HA
	# this speeds up an average contest log by about 300%, for contests like
	# ARRL DX, where only US stations are logged, it's *significantly* faster.
	
	my @dxcc;							# same format as what &dxcc returns.
	# hardcoded prefixes:
	if ($qso[$i]{'call'} =~ /^D[A-R]\d\w+$/) {							# DL
		@dxcc = qw/0 14 25 EU 0 0 0 DL/;
	}
	elsif ($qso[$i]{'call'} =~ /^[WK][A-FI-KM-Z]?[0-9][A-Z]{1,3}$/) {	# USA
		@dxcc = qw/0 5 8 NA 0 0 0 K/;
	}
	elsif ($qso[$i]{'call'} =~ /^(U[AEF]|R[A-Z])[1-4][A-Z]{1,3}$/) {	# UA
		@dxcc = qw/0 16 30 EU 0 0 0 UA/;
	}
	elsif ($qso[$i]{'call'} =~ /^([MG]|2E)\d[A-Z]{1,3}$/) {				# G
		@dxcc = qw/0 14 27 EU 0 0 0 G/;
	}
	elsif ($qso[$i]{'call'} =~ /^O(K|L)\d[A-Z]{1,3}$/) {				# OK
		@dxcc = qw/0 15 28 EU 0 0 0 OK/;
	}
	elsif ($qso[$i]{'call'} =~ /^U[R-Z]\d[A-Z]{1,3}$/) {				# UR
		@dxcc = qw/0 16 29 EU 0 0 0 UR/;
	}
	elsif ($qso[$i]{'call'} =~ /^(U[AEF]|R[A-Z])[09][A-Z]{1,3}$/) {		# UA9
		@dxcc = qw/0 17 30 AS 0 0 0 UA9/;
	}
	elsif ($qso[$i]{'call'} =~ /^S[NOPQR]\d[A-Z]{1,3}$/) {				# SP
		@dxcc = qw/0 15 28 EU 0 0 0 SP/;
	}
	elsif ($qso[$i]{'call'} =~ /^S[A-M]\d[A-Z]{1,3}$/) {				# SM
		@dxcc = qw/0 14 18 EU 0 0 0 SM/;
	}
	elsif ($qso[$i]{'call'} =~ /^H[AG]\d[A-Z]{1,3}$/) {					# HA
		@dxcc = qw/0 15 28 EU 0 0 0 HA/;
	}
	else {										# not hardcoded -> look up
		@dxcc = &dxcc($qso[$i]{'call'});
	}
	...
	# minimum needed fields are existing, go on...
		...
		# there is no gridsquare field in YFKlog, it is saved in the remarks
		# field, if available
		if (defined($qso[$i]{"gridsquare"})) {
			unless (defined($qso[$i]{'rem'})) { 			# nothing in REM yet
				$qso[$i]{'rem'} = 'GRID:'.$qso[$i]{'gridsquare'};
			}
			else {										# remarks field exists
				$qso[$i]{'rem'} .= ' GRID'.$qso[$i]{'gridsquare'}; 
			}
			delete($qso[$i]{'gridsquare'});					# delete contest_id 
		}
...
sub choseeditqso {
	...
	`QSLS`, `QSLR`, `DXCC`, `QSLINFO` FROM log_$mycall WHERE `NR`";
    # The rest of the string now depends on the content of the @qso-array:
    $sql .= "AND `CALL` LIKE '\%$qso[0]\%' " if $qso[0];
	...
	# DATE>='...' and DATE<='...' ..
    $sql .= "AND round(`BAND`,3)='$qso[4]' " if $qso[4];
    $sql .= "AND `MODE`='$qso[5]' " if $qso[5];
    $sql .= "AND `QTH` LIKE '\%$qso[6]\%' " if $qso[6];
    $sql .= "AND `NAME`  LIKE '\%$qso[7]\%' " if $qso[7];
    $sql .= "AND `QSLS` = '$qso[8]' " if $qso[8];
    $sql .= "AND `QSLR` = '$qso[9]' " if $qso[9];
    $sql .= "AND `REM` LIKE '\%$qso[12]\%' " if $qso[12];
    $sql .= "AND `PWR`='$qso[13]' " if $qso[13];
    $sql .= "AND `DXCC`='$qso[14]' " if $qso[14];
    $sql .= "AND `PFX`='$qso[15]' " if $qso[15];
    $sql .= "AND `CONT`='$qso[16]' " if $qso[16];
    $sql .= "AND `ITUZ`='$qso[17]' " if $qso[17];
    $sql .= "AND `CQZ`='$qso[18]' " if $qso[18];
    $sql .= "AND `QSLINFO` LIKE '\%$qso[19]\%' " if $qso[19];
    $sql .= "AND `IOTA`='$qso[20]' " if $qso[20];
    $sql .= "AND `STATE`='$qso[21]' " if $qso[21];
...
my $q = $dbh->prepare("SELECT `CALL`, `DATE`, `T_ON`, `T_OFF`, `BAND`, `MODE`,
		...
		`PFX`, `CONT`, `ITUZ`, `CQZ`, `QSLINFO`, `IOTA`, `STATE`, `NR` FROM
		log_$mycall WHERE `NR`='$_[0]'");
$q->execute;
@qso = $q->fetchrow_array;
# proper format for the date (yyyy-mm-dd ->  ddmmyyyy)
...
foreach my $band (@bands) {
	...
	my $sth = $dbh->prepare("SELECT $awardtype, QSLR FROM log_$mycall WHERE 
								round(BAND,4)='$band' AND $daterange");
	$sth->execute();
	my ($dx,$qslr);
...
foreach my $key (sort keys %sumdxcc) {
	...
	foreach my $band (@bands) {
		if ($sumdxccc{$key} =~ /(^| )$band( |$)/) {		# band confirmed!
			$string .= "<td> C </td>";
		}
