...
my $yfkver = '0.2.2';	# Program Version
...
while ($status == 11) {
	...
	my  @menu = ('DXCC', 'PFX', 'CQZ', 'IOTA', 'STATE', 
			'QSO / Bands', 'QSO / Continent', 'QSO / Mode');

	# Let the user select one of the awards
	my $choice = &selectlist(\$wmain, 4,30, 8 ,20, \@menu);
	attron($wmain, COLOR_PAIR(4));
...
#				if ($key eq '9999') { next; }		# All=9999 comes last!
	...
	# For QSO / Continent and QSO / Band other queries have to be made...
		...
		if ($choice eq "5") {
			&statistics("BAND", \$wmain, $daterange);	# result: BAND -> nr of QSOs
			$filename="BAND";
		}
		elsif ($choice eq "6") {
			&statistics("CONT", \$wmain, $daterange);	# result: CONTINET ->nr of QSOs
			$filename="CONT";
		}
		elsif ($choice eq "7") {
			&statistics("MODE", \$wmain, $daterange);	# result: MODE ->nr of QSOs
			$filename="MODE";
		}
