...
my $hamlibparam = '';							# for hamlib
my $checklogs = '';								# add. logs to chk fr prev QSOs
my $lotwdetails='0';							# LOTW import details?
...
sub readw {
	...
	while (1) {										# loop infinitely
		...
		# F5 -> get frequency and mode from the transceiver
			...
			my $freq = `rigctl $hamlibparam -m $rigmodel f 2> /dev/null`;
			my $mode = `rigctl $hamlibparam -m $rigmodel m 2> /dev/null`;
...
sub callinfo {
	...
	if  (defined $prefix) {	# &wpx returns undef when callsign is invalid
		...
		unless ($newdxcc eq '0E0') {				# DXCC already wkd, show bands
...
return "QTH:                Name:           QSLs:   QSLr:   RSTs:         RSTr:         ";
	...
	return "IOTA:         STATE:     QSLrL:                        Editing QSO Nr: "
}
}
...
sub choseeditqso {
    ...
    $sql .= "AND `QSLRL`='$qso[23]' " if $qso[23];
...
my $q = $dbh->prepare("SELECT `CALL`, `DATE`, `T_ON`, `T_OFF`, `BAND`, `MODE`,
		...
		`PFX`, `CONT`, `ITUZ`, `CQZ`, `QSLINFO`, `IOTA`, `STATE`, `NR`, `QSLRL`
	   	FROM
		log_$mycall WHERE `NR`='$_[0]'");
$q->execute;
@qso = $q->fetchrow_array;
...
for (my $x=0;$x < 24;$x++) {				# iterate through all input windows 
	addstr(${$_[1]}[$x],0,0,$qso[$x]);		# add new value from @qso.
	refresh(${$_[1]}[$x]);
}
...
sub editw {
	...
	while (1) {										# loop infinitely
		...
		# F3 clears the current QSO and returns to the CALL input field.
			...
			for (0 .. 23) {					# iterate through windows 0-13
				addstr(@{$_[0]}[$_],0,0," "x80);	# clear it
				refresh(@{$_[0]}[$_]);
			}
			for (0 .. 23) {	${$_[3]}[$_] = ''; }	# clear @qso.
		...
		# Then delete it and clear all fields, like with F3.
			...
			if ($answer eq 'm') { return 2 }		# menu
				...
				for (0 .. 23) {						# iterate through windows
					addstr(@{$_[0]}[$_],0,0," "x80);	# clear it
					refresh(@{$_[0]}[$_]);
				}
				for (0 .. 23) {	${$_[3]}[$_] = ''; }	# clear @qso.
...
foreach my $band (@bands) {
	...
	my $sth = $dbh->prepare("SELECT $awardtype, QSLR, QSLRL  FROM log_$mycall WHERE 
...
sub lotwimport {
	...
	open LOG, "$filename";
	open ERROR, ">$mycall-LOTW-update-from-$filename.err";
...
#						$update .= ", GRID='$grid'" if $grid;	XXX XXX XXX
					...
					where `call`='$call' and round(`band`, 3) = '$band' and
...
printw "\n\nLooking for neccessary databases...\n";

# CALLS

my $exists = $dbh->do("SHOW TABLES FROM $dbname LIKE 'calls';");

if ($exists eq '0E0') {
	open CALLS, "db_calls.sql";
	my @calls = <CALLS>;
	close CALLS;
...
$exists = $dbh->do("SHOW TABLES FROM $dbname LIKE 'clubs';");

if ($exists eq '0E0') {
	open CLUBS, "db_clubs.sql";
	my @clubs = <CLUBS>;
	close CLUBS;
...
$exists = $dbh->do("SHOW TABLES FROM $dbname LIKE 'log_$mycall';");
...
if ($exists eq '0E0') {
	...
	$dbh->do($log) or die 
					"Couldn't create log table $mycall from db_log.sql";
	printw "Created log table $mycall from db_log.sql\n";
}
else {
...
$exists = $dbh->do("SHOW TABLES FROM $dbname LIKE 'YFKconfig';");
...
sub xplanet {
	...
	open CTY, "cty.dat" or die "Cannot find cty.dat!\n";
	while ($line = <CTY>) {
		chomp($line);
		next unless ($line =~ /^[A-Z]/);			# no data lines pse
		$line =~ s/ //g;							# remove /
		
		$prefix = (split(/:/, $line))[-2];			# DXCC prefix
		$lat = (split(/:/, $line))[-5];				# 
		$lon  = (split(/:/, $line))[-4];			#
	

		next if (!defined($prefix) || ($prefix =~ /[*]/));	# remove WAEs
		$prefix =~ s/\///g;							# remove /
