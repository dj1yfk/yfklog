...
our $yfkver = '0.2.4';	# Program Version
...
while ($status == 8) {
	...
	my @sqso;			# This array will store the search criteria in the same
						# format as a @qso.
	for (0 .. 23) {		# initialize the array, so it is not undef
		$qso[$_] = '';
		$sqso[$_] = '';
	}
...
# 17=ITUZ 18=CQZ 19=QSLINFO 20=IOTA 21=STATE 22=NR 23=QSLRL
...
while ($status == 11) {
	...
	my $choice = &selectlist(\$wmain, 4,30, 9 ,20, \@menu);
	attron($wmain, COLOR_PAIR(4));
...
#				if ($key eq '9999') { next; }		# All=9999 comes last!
	...
	elsif ($choice eq '8') {		# Generate Marker file for xplanet
			&xplanet();
