...
if ($oldversion < 25) {
	...
	foreach $l (@logs) {
		...
		print " (none) " unless ($hasgrid || $hasoperator);
		print "to $l\n";
...
# Upgrade from Version 0.2.5 to 0.3.1. Nothing really to do.
if ($oldversion < 31) {
	$dbh->do("update YFKconfig set `Value` = '0.3.1' where
	`Name` = 'version';") or die "Unable to set version in table YFKconfig!";
	printw "Updated DB from 0.2.5 to 0.3.1.\n";
}
