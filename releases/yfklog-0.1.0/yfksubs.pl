...
# 2) The DXCC info, prefix, distance and beam heading 
...
# PG-up/down and toggle the QSL status of the selected QSO my pressing space
# bar.
#
...
sub toggleqsl {
	...
	my %changes;			# saves the changes we have made (NR => old value)
...
do {									# we start looping here
	...
	while ($lq->fetch()) {				# more QSOs available
		...
		if ($y == $aline) {					# highlight line?
			...
			if ($write) { $qslstat = $qsls }	# save QSL status, depending on
			else { $qslstat = $qslr }			# write/receive mode
		}
		addstr($win, $y, 0, $line);
...
# They re stored in a hash, where the keys are the callsigns (or manager call)
...
sub adifexport {
	...
	while (@q = $adif->fetchrow_array()) {
		...
		print ADIF "<band:".length($q[4]).">$q[4]m ";		# NB: added m
...
for my $i ( 0 .. $#qso ) {					# iterate through Array of Hashes
	...
	# minimum needed fields are existing, go on...
		...
		if (defined($qso[$i]{'freq'})) {
			...
			elsif ($val =~ /^7./) { $qso[$i]{'band'} = '40' } 
			elsif ($val =~ /^10.(0|1)/) { $qso[$i]{'band'} = '30' } 
			elsif ($val =~ /^14./) { $qso[$i]{'band'} = '20' } 
			elsif ($val =~ /^18.(1|0)/) { $qso[$i]{'band'} = '17' } 
			elsif ($val =~ /^21./) { $qso[$i]{'band'} = '15' } 
...
for my $i (0 .. $#qso) {					# iterate through Array of Hashes
	...
	# table. If not, use Name and QTH from the ADI-file if it exists. 
	...
	if (defined($qso[$i]{'name'}) || defined($qso[$i]{'qth'})) {
		...
		unless ($sth->fetch()) {		# nothing to fetch -> call is unknown!
			# Add information from ADIF to the database
			$dbh->do("INSERT INTO calls (CALL, NAME, QTH) VALUES
				('$qso[$i]{'call'}','$qso[$i]{'name'}','$qso[$i]{'qth'}');");
		}
	}
