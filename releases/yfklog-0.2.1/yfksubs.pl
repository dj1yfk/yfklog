...
my $dbname = "";								# DB name
my $onlinedata = 
			"CALL, DATE, round(BAND,2), MODE";	# Fields for online search log
...
sub lastqsos {
	...
	# date needed.
	my $l = $dbh->prepare("SELECT CALL, BAND, MODE, DATE, T_ON, NAME, QTH,
		RSTS, RSTR, QSLS, QSLR FROM log_$mycall ORDER BY NR DESC LIMIT $nr");	
	$l->execute();
	# temporary vars
...
sub callinfo {
	...
	if  (defined $prefix) {	# &wpx returns undef when callsign is invalid
		...
		my $lq = $dbh->prepare("SELECT CALL, DATE, T_ON, BAND, MODE, QSLS,
				QSLR, NAME, QTH, RSTS, RSTR from log_$mycall WHERE CALL 
...
do  {			# loop and get keyboard input
	...
	my $cq = $dbh->prepare("SELECT NR, CALL, BAND, MODE, DATE, T_ON, NAME, QTH,
		RSTS, RSTR, QSLS, QSLR FROM log_$mycall ORDER BY NR DESC LIMIT 
		$offset, $nbr");	
...
do {									# we start looping here
	my $lq = $dbh->prepare("SELECT NR, CALL, DATE, T_ON, BAND, MODE, QSLS,
			QSLR, NAME, QTH, RSTS, RSTR FROM log_$mycall WHERE CALL REGEXP 
			'^(.*/)?$call(/.*)?\$' ORDER BY DATE, T_ON LIMIT $offset, $nbr");	
...
sub adifexport {
	...
	while (@q = $adif->fetchrow_array()) {
		...
		# check if band is meters or centimeters
		if($q[4] < 1) {								# centimeters
...
for my $i ( 0 .. $#qso ) {					# iterate through Array of Hashes
	...
	# minimum needed fields are existing, go on...
		...
		if (defined($qso[$i]{"band"})) {				# band info 
			...
			unless($qso[$i]{"band"}=~/^\d{1,4}(c|C)?(m|M)$/) {
					$qso[$i]{'valid'}='0';
					$errmsg .= "band invalid, ";
			}
...
foreach my $band (@bands) {
	...
	my $sth = $dbh->prepare("SELECT $awardtype, QSLR FROM log_$mycall WHERE 
								round(BAND,1)='$band' AND $daterange");
	$sth->execute();
	my ($dx,$qslr);
...
sub statistics {
	...
	my $sth = $dbh->prepare("SELECT $type FROM log_$mycall WHERE 1");
	$sth->execute();
	my $type_item;
