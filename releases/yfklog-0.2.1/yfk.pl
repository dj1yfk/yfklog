...
my $yfkver = '0.2.1';	# Program Version
...
while ($status == 9) {
	...
	if ($nr > 0) {									# OK, QSOs imported
		...
		addstr($wmain,  7,10, "No QSOs imported. Probably wrong line-breaks in ADIF-file.");
		addstr($wmain,  9,12, "Try converting the linebreaks with 'dos2unix' or open ");
		addstr($wmain,  10,13, "ADIF-File in vim and enter :ff=unix <enter> :wq .");
	}
		refresh($wmain);
...
#				if ($key eq '9999') { next; }		# All=9999 comes last!
	...
	# For QSO / Continent and QSO / Band other queries have to be made...
		...
		if ($choice eq "5") {
			&statistics("BAND", \$wmain);	# result: BAND -> nr of QSOs
			$filename="BAND";
		}
		elsif ($choice eq "6") {
			&statistics("CONT", \$wmain);	# result: CONTINET ->nr of QSOs
			$filename="CONT";
		}
