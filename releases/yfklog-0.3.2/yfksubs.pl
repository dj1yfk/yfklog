...
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw( wpx dxcc makewindow clearinputfields qsotofields saveqso
readw lastqsos callinfo getdate gettime splashscreen choseqso getqso
chosepqso entrymask fkeyline winfomask selectlist askbox toggleqsl
onlinelog preparelabels labeltex emptyqslqueue adifexport ftpupload
adifimport getlogs changemycall newlogtable choseeditqso geteditqso
editw updateqso checkdate awards statistics editdb editdbw savedbedit
lotwimport databaseupgrade xplanet queryrig tableexists changeconfig
readsubconfig connectdb connectrig jumpfield
);
...
if ($rigmodel && $hashamlib) {
	Hamlib::rig_set_debug(0);
	$rig = new Hamlib::Rig($rigmodel);
	$rig->set_conf("rig_pathname", $rigpath);
	$rig->open();
}
...
sub askbox {
	...
	while (1) {									# loop until beer is empty
		...
		if ($ch =~ /^$valid$/) {					# check if it's "legal"
			unless(($valid eq '\w') || ($valid eq '[_A-Za-z0-9\/]')) {
				$ch =~ tr/[a-z]/[A-Z]/;				# make letters uppercase
			}
...
if ($oldversion < 31) {
	...
	`Name` = 'version';") or die "Unable to set version in table YFKconfig!";
	printw "Updated DB from 0.3.0 to 0.3.1.\n";
}

# Upgrade from Version 0.2.5 to 0.3.0. Nothing really to do.
if ($oldversion < 32) {
	$dbh->do("update YFKconfig set `Value` = '0.3.2' where
	`Name` = 'version';") or die "Unable to set version in table YFKconfig!";
	printw "Updated DB from 0.3.1 to 0.3.2.\n";
}
...
#	my $mode = `rigctl $hamlibparam -m $rigmodel m 2> /dev/null`;
	...
	return unless $rigmodel;
	return unless $hashamlib;

	my ($freq, $mode, $passband);
