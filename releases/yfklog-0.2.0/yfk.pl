...
my $yfkver = '0.2.0';	# Program Version
...
while ($status == 2) {	
    ...
    "Awards/Statistics - DXCC, WAZ, WPX and IOTA",
...
while ($status == 7) {
	...
	# change text in head and help lines ...
	addstr($whead, 0,0, "YFKlog v$yfkver - ADIF Export mode - Active Logbook: \U$mycall"." " x 30);
...
while ($status == 11) {
	...
	my %resultc = ('a'=>'b');		# same, but QSL received
	...
	# Menu Items
	my  @menu = ('DXCC', 'PFX', 'CQZ', 'IOTA', 'STATE',  
			'QSO / Bands');

	# Let the user select one of the awards
	my $choice = &selectlist(\$wmain, 4,30, 7 ,20, \@menu);
	attron($wmain, COLOR_PAIR(4));
	...
	if ($choice <= 4 ) {
		...
		# values.
		&awards($daterange,$menu[$choice], \%result, \%resultc);
		addstr($wmain, 5, 30, "Band    Wkd   Cfmd");
		my $y = 5;
...
#				if ($key eq '9999') { next; }		# All=9999 comes last!
				$y++;
				addstr($wmain, $y, 30, 
						sprintf("%-4s     %-4s   %-4s" ,$key, $result{$key},
								$resultc{$key})
				);	
		}
		addstr($wmain, $y, 30, 
	...
	# For QSO / Year and QSO / Band other queries have to be made...
	# ( TBD  QSO per band only. Year later )
	else {
		my $maxqsos;						# band/year with most QSOs
		%result = &statistics("blabla");
		# Check which band has most QSOs.
		foreach my $key (reverse sort {$result{$a} <=> $result{$b}}
														keys %result) {
			$maxqsos = $result{$key}; 
			last;
		}
		# Now we know the maximum number of QSOs, so we can normalize the
		# results and make a nice printout:
		my $y = 5;
		foreach my $key (sort {$a <=> $b } keys %result)	{
			$y++;
			addstr($wmain, $y, 10, "$key ");
		  	attron($wmain, COLOR_PAIR(2));
			my $len = int(($result{$key}/$maxqsos)*50);	# length of bar
			if (($len == "0") and ($result{$key} > 0)) {	# at least one
					$len = 1;								# if QSO was made
		   	}
			addstr($wmain, $y, 15, " "x$len);
		  	attron($wmain, COLOR_PAIR(4));
			addstr($wmain, $y, 16+$len, $result{$key});
		}
	}
