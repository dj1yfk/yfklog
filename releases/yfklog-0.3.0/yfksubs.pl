...
open CTY, "cty.dat" or die "cty.dat not found. Please download it from".
...
#			$qso[$i]{$key} =~ s/'/\\'/g;				# escape '
	...
	$dbh->do($sql) or die "Insert QSL $sql failed!";
...
sub newlogtable {
	...
	my $filename = 'db_log.sql';
	if ($db eq 'sqlite') {
		$filename = 'db_log.sqlite';
	}
...
unless (&tableexists("log_$mycall")) {
	my $logtable = 'db_log.sql';

	if ($db eq 'sqlite') { $logtable = 'db_log.sqlite'; }
...
# Upgrade from Version 0.2.5 to 0.3.0. Nothing really to do.
if ($oldversion < 30) {
	$dbh->do("update YFKconfig set `Value` = '0.3.0' where
	`Name` = 'version';") or die "Unable to set version in table YFKconfig!";
	printw "Updated DB from 0.2.5 to 0.3.0.\n";
}
