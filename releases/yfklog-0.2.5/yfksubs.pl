...
my $autoqueryrig='0';							# Query rig at new QSO?
...
sub choseeditqso {
    ...
    $sql .= "AND `OPERATOR`='$qso[24]' " if $qso[24];
    $sql .= "AND `GRID`='$qso[25]' " if $qso[25];
...
foreach my $band (@bands) {
	...
	while ($sth->fetch()) {						# go through all QSOs	
		...
		$dx =~ s/[A-Z]{2}$// if ($awardtype eq 'GRID');
...
if ($oldversion < 23) {			# Update to 0.2.3 database
	...
	$dbh->do("insert into YFKconfig (`Name`, `Value`) VALUES 
					('version', '$main::yfkver');") or die "Unable to set".
...
if ($oldversion < 25) {
	foreach $l (@logs) {
		...
		printw "Added GRID and OPERATOR fields to $l.\n";
	} 
...
sub queryrig {
		...
		# check if anything was returned. if not, rigctl cannot be found,
		# then exit.
