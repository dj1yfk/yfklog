...
#     EXE_FILES => [q[yfk]]
#     NAME => q[yfk]
#     PM_FILTER => q[sed -e "s|/usr/local|$(PREFIX)|"]
#     PREREQ_PM => { DBI=>q[0] }
#     VERSION_FROM => q[yfk]
...
TO_INST_PM = yfksubs.pl

PM_TO_BLIB = yfksubs.pl \
	$(INST_LIB)/yfksubs.pl
...
MACROEND = 
USEMAKEFILE = -f
PM_FILTER = sed -e "s|/usr/local|$(PREFIX)|"
...
$(INST_SCRIPT)/yfk : yfk $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/yfk
	$(CP) yfk $(INST_SCRIPT)/yfk
	$(FIXIN) $(INST_SCRIPT)/yfk
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/yfk
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                yfk' >> META_new.yml
